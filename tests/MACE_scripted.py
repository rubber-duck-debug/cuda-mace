...
def optimize_cuda_mace(model: MACE) -> None:
    ...
    for i in range(n_layers):
        ...
        model.interactions[i].linear = linear_to_cuda(
        ...
        irreps_in = o3.Irreps(
            model.products[i].symmetric_contractions.irreps_in)
        coupling_irreps = o3.Irreps([irrep.ir for irrep in irreps_in])
        irreps_out = o3.Irreps(
            model.products[i].symmetric_contractions.irreps_out)
...
def invariant_residual_interaction_forward(
    ...
    message = self.tp.forward(
        node_feats,
        edge_attrs,
        tp_weights.view(tp_weights.shape[0], -1,
                        node_feats.shape[-1]).contiguous(),
        sender.int(),
        receiver.int(),
        num_nodes,
    ).float()
...
def main(args=None):
    ...
    if args.benchmark:
        ...
        print("times opt", np.array(times_opt), ((1000.0 / times_opt) * 86400) / 1000000.0)
        print("times org", times_org, ((1000.0 / times_org) * 86400) / 1000000.0)
