...
class TensorProductReference(torch.nn.Module):
    def __init__(
        ...
        for i, ins in enumerate(self.instructions):
            ...
            ordering.append(np.arange(l3 * 2 + 1))
            shifts.append(shift)
            shift += l3 * 2 + 1

            print (ins.i_out, l3 * 2 + 1)

            out.insert(ins.i_out, i)
        ...
        ordering = [ordering[i] + shifts[i] for i in out]

        #print (shifts)
        print (ordering)

        test = torch.cat([torch.tensor(o) for o in ordering]).cuda().int()
    ...
    def forward_unweighted_tp(self, x, y):
        ...
        for i in range(x.shape[0]):  # loop over edges
            ...
            for ins, (mu1, mu2, mu3), cg_sparse in zip(
                ...
                #print (f"edge: {i}, output: {output.shape} ins.i_out: {ins.i_out}")
...
irreps1, target_irreps = o3.Irreps(f"{nchannels}x0e + {nchannels}x1o"), o3.Irreps(f"{nchannels}x0e + {nchannels}x1o + {nchannels}x2e + {nchannels}x3o")
