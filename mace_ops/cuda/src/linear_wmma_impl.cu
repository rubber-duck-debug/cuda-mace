...
__global__ void __launch_bounds__(NWARPS *WARP_SIZE) linear_wmma_kernel(const float *__restrict__ X, const float *__restrict__ W, float *__restrict__ OUT, const int NNODES, const uint M, const uint N, const uint K, const uint L)
{
...
torch::Tensor linear_wmma(torch::Tensor X, torch::Tensor W)
{
    ...
    for (int l = 0; l < 4; l++)
    {
        ...
        if (N >= 128)
        {
            linear_wmma_kernel<8><<<griddim, blockDim, shared_size, stream>>>(X.data_ptr<float>(), W.data_ptr<float>(), output.data_ptr<float>(), NNODES, M, N, K, l);
        }
        else if (N == 96)
        {
            linear_wmma_kernel<6><<<griddim, blockDim, shared_size, stream>>>(X.data_ptr<float>(), W.data_ptr<float>(), output.data_ptr<float>(), NNODES, M, N, K, l);
        }
        else if (N == 64)
        {
            linear_wmma_kernel<4><<<griddim, blockDim, shared_size, stream>>>(X.data_ptr<float>(), W.data_ptr<float>(), output.data_ptr<float>(), NNODES, M, N, K, l);
        }
        else if (N == 32)
        {
            linear_wmma_kernel<2><<<griddim, blockDim, shared_size, stream>>>(X.data_ptr<float>(), W.data_ptr<float>(), output.data_ptr<float>(), NNODES, M, N, K, l);
        }
...
template <const int NWARPS>
__global__ void __launch_bounds__(NWARPS *WARP_SIZE) elemental_linear_wmma_kernel(
    ...
    for (int element_id = 0; element_id < elemental_embedding.size(1); element_id++)
    {
        ...
        if (nselected > 0)
        {
            ...
            for (int l = 0; l < 4; l++)
            {
                ...
                if (N >= 128)
                {
                    elemental_linear_wmma_kernel<8><<<griddim, blockDim, shared_size, stream>>>(
                        X.data_ptr<float>(),
                        W.data_ptr<float>(),
                ...
                {
                    elemental_linear_wmma_kernel<6><<<griddim, blockDim, shared_size, stream>>>(
                        X.data_ptr<float>(),
                        W.data_ptr<float>(),
                        node_idx.data_ptr<int64_t>(),
                        nselected,
                ...
                {
                    elemental_linear_wmma_kernel<4><<<griddim, blockDim, shared_size, stream>>>(
                        X.data_ptr<float>(),
                        W.data_ptr<float>(),
                        node_idx.data_ptr<int64_t>(),
                        nselected,
                ...
                {
                    elemental_linear_wmma_kernel<2><<<griddim, blockDim, shared_size, stream>>>(
                        X.data_ptr<float>(),
                        W.data_ptr<float>(),
                        node_idx.data_ptr<int64_t>(),
                        nselected,
