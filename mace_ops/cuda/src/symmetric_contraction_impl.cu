    ...
    const bool requires_grad)
{
    ...
    volatile scalar_t *buffer_W3 = shared_array<scalar_t>(w3_size * blockDim.x, sptr, &space);
    volatile scalar_t *buffer_W2 = shared_array<scalar_t>(w2_size * blockDim.x, sptr, &space);
    volatile scalar_t *buffer_W1 = shared_array<scalar_t>(w1_size * blockDim.x, sptr, &space);
    ...
    const bool requires_grad)
{
    ...
    volatile scalar_t *buffer_X = shared_array<scalar_t>(blockDim.z * blockDim.x * nl, sptr, &space);
    volatile scalar_t *buffer_out = shared_array<scalar_t>(blockDim.y * blockDim.x, sptr, &space);

    volatile scalar_t *buffer_W3 = shared_array<scalar_t>(W3.size(2) * blockDim.x, sptr, &space);
    volatile scalar_t *buffer_W2 = shared_array<scalar_t>(W2.size(2) * blockDim.x, sptr, &space);
    volatile scalar_t *buffer_W1 = shared_array<scalar_t>(W1.size(2) * blockDim.x, sptr, &space);

    volatile float *buffer_U3_values_1 = shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &space);
    // volatile float *buffer_U3_values_2 = shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr);
    volatile float *buffer_U3_values_3 = shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &space);

    volatile float *buffer_U2_values_1 = shared_array<float>(nl * nl, sptr, &space);
    // volatile float *buffer_U2_values_2 = shared_array<float>(nl * nl, sptr);

    volatile short *buffer_U3_num_nonzero_1 = shared_array<short>(nl * nl, sptr, &space);
    // volatile short *buffer_U3_num_nonzero_2 = shared_array<short>(nl * nl, sptr);
    volatile short *buffer_U3_num_nonzero_3 = shared_array<short>(nl * nl, sptr, &space);

    volatile short *buffer_U3_indices_1 = shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &space);
    // volatile short *buffer_U3_indices_2 = shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr);
    volatile short *buffer_U3_indices_3 = shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &space);

    volatile short *buffer_U2_num_nonzero_1 = shared_array<short>(nl * nl, sptr, &space);
    // volatile short *buffer_U2_num_nonzero_2 = shared_array<short>(nl * nl, sptr);

    volatile short *buffer_U2_indices_1 = shared_array<short>(nl * nl, sptr, &space);
