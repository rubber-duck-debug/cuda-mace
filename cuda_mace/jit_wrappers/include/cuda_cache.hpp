...
// Helper to stringify non-type template arguments
template <typename T>
std::string valueToString() {
    // Handle `std::integral_constant` types (true/false values)
    if constexpr (std::is_same_v<T, std::integral_constant<bool, true>>) {
        return "true";
    } else if constexpr (std::is_same_v<T, std::integral_constant<bool, false>>) {
        return "false";
    } else if constexpr (std::is_integral_v<T> || std::is_floating_point_v<T> || std::is_pointer_v<T>) {
        // Handle integral, floating-point, or pointer types by converting to string
        return typeName<T>();  // Just print the type name (e.g., float, int)
    } else if constexpr (is_integral_constant<T>::value) {
        // Handle general `std::integral_constant`
...
// Variadic template function to build the template parameter list
template <typename T, typename... Ts>
void buildTemplateParams(std::string& base) {
    // Use valueToString to handle both type and non-type template parameters
    base += valueToString<T>();

    // Add a comma if there are more parameters to process
    if constexpr (sizeof...(Ts) > 0) {
        base += ", ";
        buildTemplateParams<Ts...>(base);
    }
}

// Helper function to start building the template parameters
template <typename T, typename... Ts>
std::string buildTemplateParams() {
    std::string result;
    buildTemplateParams<T, Ts...>(result);
    return result;
}
...
std::string getKernelName(const std::string& fn_name) {
    if constexpr (sizeof...(Args) > 0) {
        ...
        return fn_name; // No template parameters
    }
}
