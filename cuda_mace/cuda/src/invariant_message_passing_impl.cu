...
__global__ void calculate_first_occurences_kernel_ptr(
    const int *__restrict__ receiver_list, const int nelements_input,
    const int nelements_output, const int *sort_idx, const bool use_sort,
  ...
  for (int i = 2 * threadIdx.x + 1; i < NELEMENTS_PER_BLOCK + 1;
       i += 2 * blockDim.x) {
    int idx = block_start + i;
  ...
  // deal with 0th and last element specifically, so we dont need to use
  // torch::zeros
  ...
  for (int feature = threadCol; feature < nchannels;
    ...
    for (int m = threadRow; m < 16; m += NWARPS_PER_BLOCK * TM) {
      ...
      for (int edge = edge_start; edge < edge_end; edge++) {
        ...
        for (int i = 0; i < TN; i++) {
          for (int j = 0; j < TM; j++) {
            regOut[i * TM + j] += regX[i] * regY[j] * regRadial[i * TM + j];
          }
        }
      }
