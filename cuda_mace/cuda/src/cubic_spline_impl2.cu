...
template __global__ void evaluate_spline_kernel_ptr<float, true>(
    float *r, float *r_knots, float *coeff, const int nsamples,
    const int nknots, const int noutputs, const float r_width,
    const float r_max, float *R_out, float *R_deriv);
template __global__ void evaluate_spline_kernel_ptr<float, false>(
    float *r, float *r_knots, float *coeff, const int nsamples,
    const int nknots, const int noutputs, const float r_width,
    const float r_max, float *R_out, float *R_deriv);
...
template __global__ void backward_spline_kernel_ptr<float>(float *grad_output,
                                                           float *R_deriv,
                                                           float *r_grad,
                                                           const int nsamples,
                                                           const int noutputs);
