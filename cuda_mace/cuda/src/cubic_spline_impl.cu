...
template <typename scalar_t>
__global__ void backward_spline_kernel_ptr(
    scalar_t *grad_output, scalar_t *R_deriv, scalar_t r_grad,
    const int nsamples, const int noutputs) {
  ...
  for (int j = laneID; j < noutputs; j += WARP_SIZE * 4) {
    ...
    for (int l = 0; l < 4; l++) {
      reg_r_deriv[l] = R_deriv[i * 4 * noutputs + l * WARP_SIZE + j];
      reg_grad_output[l] = grad_output[i * 4 * noutputs + l * WARP_SIZE + j];
    }
