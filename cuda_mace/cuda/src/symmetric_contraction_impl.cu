...
symmetric_contraction_L0_forwards_kernel(
    ...
    const torch::PackedTensorAccessor32<int, 1, torch::RestrictPtrTraits>
        atom_types,
    const torch::PackedTensorAccessor32<short, 3, torch::RestrictPtrTraits>
        U3_num_nonzero,
    const torch::PackedTensorAccessor32<int, 3, torch::RestrictPtrTraits>
        U3_indices,
    ...
    const torch::PackedTensorAccessor32<short, 2, torch::RestrictPtrTraits>
        U1_num_nonzero,
    const torch::PackedTensorAccessor32<short, 2, torch::RestrictPtrTraits>
        U1_indices,
    ...
    const torch::PackedTensorAccessor32<scalar_t, 4, torch::RestrictPtrTraits>
        W1,
    const int w3_size, const int w2_size, const int w1_size,
...
std::vector<torch::Tensor> symmetric_contraction_L0_forwards_gpu(
    torch::Tensor X, torch::Tensor atom_types, torch::Tensor U3_num_nonzero,
    ...
    const int64_t nthreadY = 4, const int64_t nthreadZ = 1)
{
  ...
  AT_DISPATCH_FLOATING_TYPES(
      X.type(), "symmetric_contraction_L0_forwards_gpu", ([&]
        ...
        shared_array<float>(u3_n_nonsparse[0].item<int>() * nl * nl, sptr,
                            &shared_size);
        ...
        shared_array<int>(u3_n_nonsparse[0].item<int>() * nl * nl, sptr,
                          &shared_size);
        ...
        // shared_array<short>(nl, sptr, &shared_size);
        symmetric_contraction_L0_forwards_kernel<<<block_dim, grid,
                                                   shared_size>>>(
            X.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(),
            atom_types.packed_accessor32<int, 1, torch::RestrictPtrTraits>(),
            U3_num_nonzero
                .packed_accessor32<short, 3, torch::RestrictPtrTraits>(),
            U3_indices.packed_accessor32<
                int, 3, torch::RestrictPtrTraits>(), // L=0 specific
            U3_values.packed_accessor32<
                float, 3, torch::RestrictPtrTraits>(), // L=0 specific
            U2_num_nonzero
                .packed_accessor32<short, 3, torch::RestrictPtrTraits>(),
...
__global__ void symmetric_contraction_LGT0_forwards_kernel(
    ...
    const torch::PackedTensorAccessor32<short, 4, torch::RestrictPtrTraits>
        U3_indices_1,
    const torch::PackedTensorAccessor32<short, 4, torch::RestrictPtrTraits>
        U3_indices_2,
    const torch::PackedTensorAccessor32<short, 4, torch::RestrictPtrTraits>
    ...
    const bool requires_grad)
{
  ...
  volatile scalar_t *buffer_W3 =
      shared_array<scalar_t>(W3.size(2) * blockDim.x, sptr, &space);
  volatile scalar_t *buffer_W2 =
      shared_array<scalar_t>(W2.size(2) * blockDim.x, sptr, &space);
  volatile scalar_t *buffer_W1 =
      shared_array<scalar_t>(W1.size(2) * blockDim.x, sptr, &space);

  volatile float *buffer_U3_values_1 =
      shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &space);
  // volatile float *buffer_U3_values_2 = shared_array<float>(u3_maxn_nonsparse
  // * nl * nl, sptr);
  volatile float *buffer_U3_values_3 =
      shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &space);

  volatile float *buffer_U2_values_1 =
      shared_array<float>(nl * nl, sptr, &space);
  // volatile float *buffer_U2_values_2 = shared_array<float>(nl * nl, sptr);

  volatile short *buffer_U3_num_nonzero_1 =
      shared_array<short>(nl * nl, sptr, &space);
  // volatile short *buffer_U3_num_nonzero_2 = shared_array<short>(nl * nl,
  // sptr);
  volatile short *buffer_U3_num_nonzero_3 =
      shared_array<short>(nl * nl, sptr, &space);

  volatile short *buffer_U3_indices_1 =
      shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &space);
  // volatile short *buffer_U3_indices_2 = shared_array<short>(u3_maxn_nonsparse
  // * nl * nl, sptr);
  volatile short *buffer_U3_indices_3 =
      shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &space);

  volatile short *buffer_U2_num_nonzero_1 =
      shared_array<short>(nl * nl, sptr, &space);
  // volatile short *buffer_U2_num_nonzero_2 = shared_array<short>(nl * nl,
  // sptr);

  volatile short *buffer_U2_indices_1 =
      shared_array<short>(nl * nl, sptr, &space);
  ...
  for (int ii = threadIdx.z; ii < NATOMS_PER_BLOCK; ii += blockDim.z)
  {
    ...
    for (int i = threadIdx.y; i < nl; i += blockDim.y)
    {
      ...
      for (int j = 0; j < nl; j++)
      {
        ...
        for (int k = 0; k < nk3; k++)
        {
          ...
          scalar_t X_kdx = buffer_X[threadIdx.z * (nl * blockDim.x) +
                                    kdx * blockDim.x + threadIdx.x];
        ...
        output2 += (output3 + uw2) * buffer_X[threadIdx.z * (nl * blockDim.x) +
      ...
      if (requires_grad)
      {
        for (int j = 0; j < nl; j++)
        {
          ...
          dB += dBi * buffer_X[threadIdx.z * (nl * blockDim.x) +
                               j * blockDim.x + threadIdx.x];
        }
      }
      ...
      if (requires_grad)
      {
        ...
        if (threadIdx.x == 0) {
            grad[atom_id][lout][threadIdx.z -> i][threadIdx.y -> channel_id] =
        dB;
            }
        }*/
    ...
    atomicAdd((scalar_t *)&buffer_out[threadIdx.y * blockDim.x + threadIdx.x],
              output_1);
...
std::vector<torch::Tensor> symmetric_contraction_LGT0_forwards_gpu(
    torch::Tensor X, torch::Tensor atom_types, torch::Tensor U3_num_nonzero_1,
    torch::Tensor U3_num_nonzero_2, torch::Tensor U3_num_nonzero_3,
    torch::Tensor U3_indices_0, torch::Tensor U3_indices_1,
    torch::Tensor U3_indices_2, torch::Tensor U3_indices_3,
    torch::Tensor U3_values_0, torch::Tensor U3_values_1,
    torch::Tensor U3_values_2, torch::Tensor U3_values_3,
    torch::Tensor U2_num_nonzero_1, torch::Tensor U2_num_nonzero_2,
    torch::Tensor U2_indices_1, torch::Tensor U2_indices_2,
    torch::Tensor U2_values_1, torch::Tensor U2_values_2,
    torch::Tensor U1_num_nonzero, torch::Tensor U1_index, torch::Tensor W3,
    torch::Tensor W2, torch::Tensor W1, const int64_t W3_l0_size,
    const int64_t W2_l0_size, const int64_t W1_l0_size,
    ...
    const int64_t nthreadZ = 1)
{
  ...
  dim3 gridDim(find_num_blocks(natoms, NATOMS_PER_BLOCK), nchannels / nthreadX,
               nlout - 1);
  ...
  AT_DISPATCH_FLOATING_TYPES(
      X.type(), "symmetric_contraction_LGT0_forwards_gpu", ([&]
        ...
        shared_array<scalar_t>(nthreadX * nl, sptr, &shared_size);
        shared_array<scalar_t>(nthreadY * nthreadX, sptr, &shared_size);
        shared_array<float>(U3_max_nonsparse[0].item<int>() * nl * nl, sptr,
                            &shared_size);
        shared_array<float>(nl * nl, sptr, &shared_size);
        ...
        shared_array<int>(U3_max_nonsparse[0].item<int>() * nl * nl, sptr,
                          &shared_size);
        shared_array<short>(nl * nl, sptr, &shared_size);
        shared_array<short>(nl * nl, sptr, &shared_size);
        shared_array<short>(nl * nl, sptr, &shared_size);
        ...
        symmetric_contraction_L0_forwards_kernel<
            scalar_t><<<gridDim_0, blockDim_0, shared_size>>>(
            X.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(),
            atom_types.packed_accessor32<int, 1, torch::RestrictPtrTraits>(),
            U3_num_nonzero_1
                .packed_accessor32<short, 3, torch::RestrictPtrTraits>(),
            U3_indices_0.packed_accessor32<
                int, 3, torch::RestrictPtrTraits>(), // L = 0 specific
            U3_values_0.packed_accessor32<
                float, 3, torch::RestrictPtrTraits>(), // L = 0 specific
            U2_num_nonzero_1
                .packed_accessor32<short, 3, torch::RestrictPtrTraits>(),
            U2_indices_1
            ...
            W2.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            W1.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            W3_l0_size, W2_l0_size, W1_l0_size, U3_max_nonsparse[0].item<int>(),
            output.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(),
            grad.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
            X.requires_grad());
        ...
        shared_array<scalar_t>(blockDim.z * blockDim.x * nl, sptr,
                               &shared_size);
        ...
        // shared_array<float>(u3_maxn_nonsparse * blockDim.y * nl, sptr,
        // &shared_size);
        shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &shared_size);
        ...
        // shared_array<short>(u3_maxn_nonsparse * blockDim.y * nl, sptr,
        // &shared_size);
        shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &shared_size);
        ...
        symmetric_contraction_LGT0_forwards_kernel<
            scalar_t><<<gridDim, blockDim, shared_size>>>(
            X.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(),
            atom_types.packed_accessor32<int, 1, torch::RestrictPtrTraits>(),
            U3_num_nonzero_1
                .packed_accessor32<short, 3, torch::RestrictPtrTraits>(),
            ...
            U3_indices_1
                .packed_accessor32<short, 4, torch::RestrictPtrTraits>(),
            U3_indices_2
                .packed_accessor32<short, 4, torch::RestrictPtrTraits>(),
            U3_indices_3
                .packed_accessor32<short, 4, torch::RestrictPtrTraits>(),
...
int64_t LGT0_shared_memory_required(int64_t nthreadsX, int64_t nthreadsY,
                                    int64_t nthreadsZ,
                                    ...
                                    torch::ScalarType scalar_type)
{
  ...
  shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &shared_size);
  // shared_array<float>(u3_maxn_nonsparse * nthreadsY * nl, sptr,
  // &shared_size);
  shared_array<float>(u3_maxn_nonsparse * nl * nl, sptr, &shared_size);
  ...
  shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &shared_size);
  // shared_array<short>(u3_maxn_nonsparse * nthreadsY * nl, sptr,
  // &shared_size);
  shared_array<short>(u3_maxn_nonsparse * nl * nl, sptr, &shared_size);
...
bool set_shared_mem_size(int64_t amount, torch::ScalarType scalar_type)
{
  ...
  else
  {
    ...
    case torch::ScalarType::Double:
      ...
      break;
    case torch::ScalarType::Float:
      cudaFuncSetAttribute(symmetric_contraction_LGT0_forwards_kernel<float>,
                           cudaFuncAttributeMaxDynamicSharedMemorySize, amount);
      break;
    }
  }
