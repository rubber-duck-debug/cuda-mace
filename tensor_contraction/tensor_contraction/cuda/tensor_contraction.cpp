...
torch::Tensor get_Uw3_sparse_contraction_tc(torch::Tensor U_values, torch::Tensor U_indices, torch::Tensor U_nvals, torch::Tensor Uw_nvals, torch::Tensor W) {

	auto options = torch::TensorOptions().dtype(torch::kFloat32).layout(torch::kStrided).device(torch::kCUDA);

	torch::Tensor output = torch::zeros( { U_values.size(0), U_values.size(1), U_values.size(2), W.size(1) }, options);

	Uw3_wmma_sparse_contraction_tensorcore_kernel_16x16_f32(U_values, U_indices, U_nvals, Uw_nvals, W, output);

	return output;
}

torch::Tensor get_Uw3_sparse_contraction(torch::Tensor U_values, torch::Tensor U_indices, torch::Tensor U_nvals, torch::Tensor Uw_indices,
		torch::Tensor Uw_nvals, torch::Tensor W) {

	auto options = torch::TensorOptions().dtype(torch::kFloat32).layout(torch::kStrided).device(torch::kCUDA);

	torch::Tensor output = torch::zeros( { U_values.size(0), U_values.size(1), U_values.size(2), W.size(1) }, options);

	Uw3_sparse_contraction_kernel_16x16_f32(U_values, U_indices, U_nvals, Uw_indices, Uw_nvals, W, output);

	return output;
}

torch::Tensor get_UwN3_sparse(torch::Tensor Uw, torch::Tensor U_indexes, torch::Tensor nvals, torch::Tensor features, const int nblocksx, const int nblocksy,
		const int nblocksz, const int nthreadsx, const int nthreadsy) {
	...
	torch::Tensor output = torch::zeros( { natoms, nirreps, nirreps, nfeatures }, options);

	UwN3_sparse_contraction(Uw, U_indexes, nvals, features, output, nblocksx, nblocksy, nblocksz, nthreadsx, nthreadsy);

	return output;
}

torch::Tensor get_UwN2_dense_contraction(torch::Tensor Uw2, torch::Tensor features, const int nblocksx, const int nblocksy, const int nthreadsx,
		const int nthreadsy) {
	...
	torch::Tensor output = torch::zeros( { natoms, nfeatures, nirreps }, options);

	UwN2_dense_contraction(Uw2, features, output, nblocksx, nblocksy, nthreadsx, nthreadsy);
