...
__global__ void compute_gemm(const half *A, const half *B, const float *C, float *D, float alpha, float beta) {
	extern __shared__ half
	shmem[][CHUNK_K * K + SKEW_HALF];
...
#pragma unroll
		for (int tile_k = 0; tile_k < K_TILES; tile_k += CHUNK_K) {
			...
			size_t shmem_idx =
					warpId < (WARPS_PER_BLOCK / 2) ? (M * (warpId % (WARPS_PER_BLOCK / 2)) * 2) : (N * (warpId % (WARPS_PER_BLOCK / 2)) * 2 + shmem_idx_b_off);
...
#pragma unroll
		for (int i = 0; i < K; i++) {
			*((int4*) (dst_gmem_warp_stream_ptr + GLOBAL_MEM_STRIDE * i) + laneId) = *((int4*) (shmem_warp_stream_ptr + SHMEM_STRIDE * i) + laneId);
		}
...
int main(int argc, char **argv) {
	...
	cudaMallocManaged((void**) &A_h, sizeof(float) * M_GLOBAL * K_GLOBAL);
	cudaMallocManaged((void**) &B_h, sizeof(float) * K_GLOBAL * N_GLOBAL);
	cudaMallocManaged((void**) &C_h, sizeof(float) * M_GLOBAL * N_GLOBAL);
	...
	cudaMalloc((void**) &C, sizeof(float) * M_GLOBAL * N_GLOBAL);
	cudaMalloc((void**) &D, sizeof(float) * M_GLOBAL * N_GLOBAL);
